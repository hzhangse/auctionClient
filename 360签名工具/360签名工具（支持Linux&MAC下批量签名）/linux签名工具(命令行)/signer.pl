...
sub get_system_type(){
    ...
    return  `getconf LONG_BIT` == 32 ? "32" : "64";
}
...
sub del_sig{
    ...
    my $type    = shift;
    my $ori_apk = shift;
    my $zip =  "./tools/bin/7za$type -tzip -ssc d $ori_apk \"META-INF\" ";
    my $ret = `$zip`;
	
    `chmod 777 ./tools/bin/7za$type`;
...
sub sign_error_msg{
    ...
    my $jarsigner = "./tools/bin/jarsigner$type -digestalg SHA1 -sigalg MD5withRSA -verbose -keystore $keystore -storepass $storepass -keypass $keypass -signedjar $signed_apk $ori_apk \"$alias\" "; 
    
    `chmod 777 ./tools/bin/jarsigner$type`;
...
sub align{
    ...
    my $aligned = "./tools/bin/zipalign$type -f -v 4 $signed_apk $aligned_apk";

    `chmod 777 ./tools/bin/zipalign$type`;
    my $ret_str = `$aligned`;	
...
sub signer{
    ...
    my $del_ret = del_sig($type, $apk_name);	
    if( $del_ret ne 0 ){
       `chmod 777 $apk_name`;
    }

    my $sign_ret = sign_error_msg($apk_name, $type, $signed_apk);
    if($sign_ret ne 0){
       `chmod 777 $signed_apk`;
    }
    
    my $align_ret = align($type, $signed_apk, $aligned_apk);
    if( $align_ret ne 0 ){
       `rm -rf $signed_apk`;
